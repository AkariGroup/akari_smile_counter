...
# https://github.com/google/mediapipe/blob/master/mediapipe/modules/face_geometry/data/geometry_pipeline_metadata_landmarks.pbtxt
# canonical_mesh:
...
# canonical_metric_landmarks.shape: (3, 468)
...
def get_metric_landmarks(screen_landmarks, pcf):
    # screen_landmarks.shape is either (3,468) or (3,478) depending on the model (with attention or not)
...
# https://github.com/google/mediapipe/blob/master/mediapipe/modules/face_geometry/libs/procrustes_solver.cc
...
def solve_weighted_orthogonal_problem(source_points, target_points, point_weights):
    ...
    transform_mat = internal_solve_weighted_orthogonal_problem(
        source_points, target_points[:, :468], sqrt_weights
    )
...
def internal_solve_weighted_orthogonal_problem(sources, targets, sqrt_weights):
    ...
    source_center_of_mass = np.sum(
        twice_weighted_sources, axis=1) / total_weight
    ...
    pointwise_diffs = weighted_targets - \
        np.matmul(rotation_and_scale, weighted_sources)
